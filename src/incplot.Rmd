...
incplotfunc <- function(timevar, eventvar, data1, data2, ylimmin = 60, name, lasmy = 1) {
  ...
  if (!cat2) {
    if (lasmy == 1){par(mar = c(4, 4, .5, 0) + 0.1)}
    if (lasmy == 2){par(mar = c(8, 4, .5, 0) + 0.1)}
  }
  ...
  axis(1,
    at = mean(b), name,
    tick = F, line = ifelse(cat2|lasmy == 2, 6, 2),
    cex.axis = cexmy
  )
  ...
  text(
    ...
    cex = ifelse(cat2, cexmy, cexmy * 0.8)
  )

  if (!cat2) {
    legend("topleft",
      horiz = TRUE,
      c(levels(incdata$shf_ef_cat), "1yr", "5yr"),
      col = c(global_cols[c(2, 5, 8)], "grey", "grey"),
      fill = c(global_cols[c(2, 5, 8)], "grey", "grey"),
      density = c(-20, -20, -20, -20, 40),
      border = "white",
      bty = "n",
      cex = cexmy
    )
