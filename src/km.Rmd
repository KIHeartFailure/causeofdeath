...
kmfunc <- function(val, eventname, ymax = 1) {
  ...
  axis(1, at = seq(0, 14, 2) * 365, seq(0, 14, 2), cex.axis = cexmy)
  plotrix::axis.break(2, ymax + 0.025, style = "slash")

  levs <- levels(rsdata %>% pull(shf_ef_cat))

  legend(x = 5.5 * 365, y = ymax + 0.05, c("HR (95% CI), p-value", hrprint), cex = cexmy, adj = 0.5, bty = "n")
  legend(
  ...
  mtext("No. at risk", side = 1, line = 3.7, at = -990, adj = 0, cex = cexmy, font = 2)
  mtext(levs[1], side = 1, line = 4.7, at = -990, adj = 0, cex = cexmy, col = global_cols[2])
  mtext(levs[2], side = 1, line = 5.7, at = -990, adj = 0, cex = cexmy, col = global_cols[5])
  mtext(levs[3], side = 1, line = 6.7, at = -990, adj = 0, cex = cexmy, col = global_cols[8])

  nrisk <- summary(fits, seq(0, 14, 2) * 365, extend = T)

  axis(1, at = seq(0, 14, 2) * 365, labels = nrisk$n.risk[nrisk$strata == paste0("shf_ef_cat=", levs[1])], line = 3.7, tick = FALSE, cex.axis = cexmy)
  axis(1, at = seq(0, 14, 2) * 365, labels = nrisk$n.risk[nrisk$strata == paste0("shf_ef_cat=", levs[2])], line = 4.7, tick = FALSE, cex.axis = cexmy)
  axis(1, at = seq(0, 14, 2) * 365, labels = nrisk$n.risk[nrisk$strata == paste0("shf_ef_cat=", levs[3])], line = 5.7, tick = FALSE, cex.axis = cexmy)
}
```
