...
survfunc <- function(eventvar, timevar, val, eventname, compevent) {
  ...
  out[2, 2] <- "Crude SHR (95% CI), p-value"
  ...
  mod <- summary(z <- crr(rsdata$sos_outtime_death,
    rsdata %>% pull(!!sym(compevent)),
    rsdata %>% select(shf_ef_cat_cr_HFmrEF, shf_ef_cat_cr_HFpEF, shf_sex_cr_Male, shf_age),
    failcode = 1, cencode = 0
  ))
  ...
  out[3, 2] <- "Adjusted age and sex SHR (95% CI), p-value"
  ...
  out[4, 2] <- "Crude HR (95% CI), p-value"
  ...
  out[5, 2] <- "Adjusted age and sex HR (95% CI), p-value"
